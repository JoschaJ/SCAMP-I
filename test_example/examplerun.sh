...
run_scatter_mcmc.py -f ${configfile} -o logs/${basename}_inputlog.csv -w output/chains -t ${taumcruntime} -nw ${tau_nwalkers} -m ${model} --showprogress
check_tau_chains.py -i logs/${basename}_inputlog.csv -w output/diagnostic_plots
...
define_tau_passfail_burnin.py -i logs/${basename}_inputlog.csv -o ${basename}_outputlog.csv -w logs
parameter_extraction.py -i logs/${basename}_outputlog.csv -o ${basename}_output.csv -w output/results
echo "Fitting for alpha..."
alpha_mcmc.py -i output/results/${basename}_output.csv -o ${basename}_output.csv --sampleswritedir output/chains --resultswritedir output/results -t ${alphamcruntime} -nw ${alpha_nwalkers} --showprogress
check_alpha_chains.py -i output/results/${basename}_output.csv -w output/diagnostic_plots
echo "Now check the alpha chain plots and decide what burnin fraction you want."
define_alpha_burnin.py -i output/results/${basename}_output.csv
best_fit_alpha_tau.py -i output/results/${basename}_output.csv
basic_plotting.py -f output/results/${basename}_output.csv -w output/results/plots
